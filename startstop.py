...
class StopOnAc1Condition(Condition):
	...
	def get_value(self):
		...
		if self.multi_service_type == 'vebus':
			...
			# acsystem only reports /Ac/ActiveIn/ActiveInput, which is 0 when input 1 is connected.
			activein = self.monitor.get_value(self.multi_service,
				'/Ac/ActiveIn/ActiveInput')
			return None if activein is None else activein == 0
...
class StopOnAc2Condition(Condition):
	...
	def get_value(self):
		if self.multi_service_type == 'vebus':
			...
			# We currently only have the Multi RS, which does not have AC input 2.
			# Do not return None, that will trigger the lossofcommunication event if this condition is active.
			return False
